...
```
type TBasketItems = Pick<IProductItem, "id"|"title"|"price">
```
...
- products: IProductItem[] - массив объектов карточек товара
...
Поля класса:
 - productList: HTMLElement - Элемент карточки товара 
 - basketButton: HTMLButtonElement - Элемент кнопки оформления заказа
 - totalPrice: HTMLElement - Элемент общей суммы заказа
...
- constructor(container: HTMLElement, events: IEvents) Конструктор принимает разметкук корзины и экземпляр класса `EventEmitter` для возможности инициации событий.
 - set products - сеттер для добавления товаров 
